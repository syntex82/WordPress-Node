...
function EmptyMediaPlaceholder({ 
  ...
  return (
    <button
      onClick={onClick}
      className="w-full h-32 border-2 border-dashed border-gray-600 hover:border-blue-500 rounded-xl flex flex-col items-center justify-center gap-2 transition-all hover:bg-gray-800/50 group"
    >
      <div className="w-12 h-12 rounded-full bg-gray-700 group-hover:bg-blue-500/20 flex items-center justify-center transition-colors">
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      {type === 'gallery' ? (
        values.length > 0 ? (
          ...
          <EmptyMediaPlaceholder type={type} onClick={() => setShowPicker(true)} />
        )
      ) : singleValue ? (
        <MediaPreview
          type={type}
