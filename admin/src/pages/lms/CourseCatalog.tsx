...
export default function CourseCatalog() {
  ...
  const getLevelBadge = (level: string) => {
    ...
    return <span className={`px-2 py-1 text-xs rounded-full ${colors[level] || 'bg-gray-100'}`}>{level.replace('_', ' ')}</span>;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            ...
            <button onClick={loadData} className="mt-2 text-red-700 hover:text-red-800 underline">Try Again</button>
          </div>
        )}
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course) => (
                <Link key={course.id} to={`/lms/course/${course.slug}`} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {course.category && <span className="text-xs text-gray-500">{course.category}</span>}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="font-bold text-lg">
                        {course.priceType === 'FREE' ? (
                          ...
                          <span>${course.priceAmount?.toFixed(2)}</span>
                        )}
                      </div>
                    </div>
                  </div>
